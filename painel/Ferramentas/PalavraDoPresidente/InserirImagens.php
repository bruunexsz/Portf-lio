...
$SelectImagens = sprintf("SELECT cadastroimagenspresidente.ID,
	cadastroimagenspresidente.ImagemConteudoDoPresidente,								
	cadastroimagenspresidente.PastaDeConteudoDoPresidente		
FROM ".BANCO.".cadastroimagenspresidente
WHERE cadastroimagenspresidente.PastaDeConteudoDoPresidente = '%s'			
ORDER BY ID asc",
				...
				echo '<table border="0" cellpadding="0" cellspacing="0">';
						...
						for($i=1;$i<=count($BuscaMostrarImagensThumbs);$i++){
							...
							$atual = 0;
								...
								$CaminhoImagensThumbs = "../../../Img/ConteudoPalavraDoPresidenteFKP/";																
								$CaminhoDaImagem = $CaminhoImagensThumbs."".$CampoPastaDeConteudo."/".$BuscaMostrarImagensThumbs[$i]["ImagemConteudo"];
								$CaminhoDaImagemInsert = CAMINHO_IMAGENS_PALAVRA_DO_PRESIDENTE."".$CampoPastaDeConteudo."/".$BuscaMostrarImagensThumbs[$i]["ImagemConteudo"];
								$CaminhoDaImagemLink = substr(CAMINHO_SITE_GERAL, 7)."Img/ConteudoPalavraDoPresidenteFKP/".$CampoPastaDeConteudo."/".$BuscaMostrarImagensThumbs[$i]["ImagemConteudo"];
								?>
        <table border="0" cellspacing="0" cellpadding="0" id="<?php echo $BuscaMostrarImagensThumbs[$i]["ID"] ?>">
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="0" style="border:solid 2px; border-color:#FFFFFF" id="<?php echo $BuscaMostrarImagensThumbs[$i]["ID"]."PalavraDoPresidente" ?>">
                <tr>
                  <td><table  border="0" cellpadding="0" cellspacing="1" bgcolor="#000000">
                      <tr>
                        <td bgcolor="#FFFFFF"><a href="javascript:SelecionarImagensBBCode('<?php echo $CaminhoDaImagemLink; ?>','<?php echo $BuscaMostrarImagensThumbs[$i]["ID"]."PalavraDoPresidente" ?>');"><img src="Ferramentas/PalavraDoPresidente/MostrarThumbs.php?Imagem=<?php echo $CaminhoDaImagem; ?>" border="0" /></a></td>
                      </tr>
                    </table></td>
                </tr>
              </table></td>
  ...
  <tr>
    <td><table width="670" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="right"><input type="button" class="BotaoPadrao" value="Excluir imagens selecionadas" onClick="ConfirmacaoExclusaoPorCheckBox('Ferramentas/PalavraDoPresidente/AcaoExcluirImagensPalavraDoPresidente.php','DivInserirImagem','IDImagens='+encodeURIComponent(document.getElementById('ValoresCheckbox').value)+'&CampoPastaDeConteudo='+encodeURIComponent('<?php echo $CampoPastaDeConteudo;  ?>'),'ValoresCheckbox');"/></td>
        </tr>
        <tr>
