...
mysql_select_db(BANCO, $ConexaoBanco) or die(mysql_error());
   ...
   $cSQL = sprintf("SELECT cadastromensagemfiliado.ID,
	...
	#INICIO SELECT ANEXOS	
			...
			ORDER BY ID asc",
			mysql_real_escape_string(utf8_decode($BuscaMostrarMensagens[1]["PastaDeConteudoDaMensagem"]))
			);			
			#echo $cSQL;
			$oRS = mysql_query($cSQL) or die (mysql_error());
			$nCount=1;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="20" class="TextoVerdana12B">Aten&ccedil;&atilde;o, voc&ecirc; ir&aacute; adicionar usu&aacute;rios para a mensagem: <?php echo utf8_encode($BuscaMostrarMensagens[1]["TituloDaMensagem"]); ?></td>
  </tr>
  <tr>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><table width="100%" border="0" cellpadding="0" cellspacing="3">
                      <tr>
                        <td class="TextoVerdana12A"><select name="ListaUsuarios[]" size="10" multiple="multiple" style="width:665px;" class="TextoListBox" id="ListaUsuarios">
                    ...
                    mysql_Free_Result($oRS);
                    ?>
                            <?php if($BuscaMostrarFiliados[$i]["ID"] != $BuscaMostrarUsuariosSelecionados[1]["IDUsuarioFiliado"]){?>
                      ...
                      <tr>
                        <td><input type="button" class="BotaoPadrao" 
                                            onClick="PegarValoresSelect('ListaUsuarios')
                                              ...
                                              EnviarFormularios('Ferramentas/AreaDoFiliado/Mensagens/AcaoEscolherUsuarios.php','DivResultadosInternos','CampoValoresListBox='+encodeURIComponent(document.getElementById('ValoresListBox').value)+'&CampoIDMensagem='+encodeURIComponent('<?php echo $BuscaMostrarMensagens[1]["ID"]; ?>'));                                  
                                            }" value="Adicionar usu&aacute;rios"></td>
                        <td><input type="button" class="BotaoPadrao" 
