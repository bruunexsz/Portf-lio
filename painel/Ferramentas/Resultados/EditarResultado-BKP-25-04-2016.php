...
$SelectResultado = sprintf("SELECT cadastroresultado.ID,
	...
	cadastroresultado.DataDeCadastroDoResultado,
	cadastroresultado.DataDoResultado,
	cadastroresultado.TituloDoResultado,
	cadastroresultado.LocalDoResultado,
	cadastroresultado.TextoConteudoDoResultado,
	cadastroresultado.IDGaleriaDeFotos
FROM ".BANCO.".cadastroresultado 
WHERE cadastroresultado.ID  = '%d'
LIMIT 1",
	FiltrarCampos(mysql_real_escape_string(utf8_decode($IDResultado)))
);
...
while ($row = mysql_fetch_array($ResultadoResultado)){			
	...
	$BuscaMostrarResultados[$nCount]["DataDeCadastroDoResultado"] = trim($row[2]);				
	$BuscaMostrarResultados[$nCount]["DataDoResultado"          ] = trim($row[3]);
	$BuscaMostrarResultados[$nCount]["TituloDoResultado"        ] = trim($row[4]);
	$BuscaMostrarResultados[$nCount]["LocalDoResultado"         ] = trim($row[5]);
	$BuscaMostrarResultados[$nCount]["TextoConteudoDoResultado" ] = trim($row[6]);	
	$BuscaMostrarResultados[$nCount]["IDGaleriaDeFotos"         ] = trim($row[7]);			
$nCount++;
}
...
$SQLNomeGalerias = sprintf("SELECT cadastrogaleria.ID,
	cadastrogaleria.AtivacaoDaGaleria,
	cadastrogaleria.DataDeCadastroDaGaleria,
	cadastrogaleria.TituloDaGaleria			
FROM ".BANCO.".cadastrogaleria
WHERE cadastrogaleria.AtivacaoDaGaleria  = '1'
AND cadastrogaleria.ID = '%d'
LIMIT 1",
	FiltrarCampos(mysql_real_escape_string(utf8_decode($BuscaMostrarResultados[1]["IDGaleriaDeFotos"])))
);

$RSNomeGalerias = mysql_query($SQLNomeGalerias) or die (mysql_error());
$nCount=1;
while ($row = mysql_fetch_array($RSNomeGalerias)){			
	$BuscaMostrarNomesGalerias[$nCount]["ID"                       ] = trim($row[0]);
	$BuscaMostrarNomesGalerias[$nCount]["AtivacaoDaGaleria"        ] = trim($row[1]);
	$BuscaMostrarNomesGalerias[$nCount]["DataDeCadastroDaGaleria"  ] = trim($row[2]);				
	$BuscaMostrarNomesGalerias[$nCount]["TituloDaGaleria"          ] = trim($row[3]);		
$nCount++;
}
mysql_Free_Result($RSNomeGalerias);
              ...
              <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><table border="0" cellpadding="0" cellspacing="3">
                      ...
                      <tr>
                        <td class="TextoVerdana12A"><?php $DataDoResultado = substr(FormataDataBanco($BuscaMostrarResultados[1]["DataDoResultado"]),0,10);			  
			  
				$ArrayParaSeparar = explode("/", $DataDoResultado);
				for($i = 0; $i < count($ArrayParaSeparar); $i++) {
			 	} ?>
                          <input type="hidden" value="<?php echo $ArrayParaSeparar[0]; ?>" id="ValorDiaDoResultado" />
                          <input type="hidden" value="<?php echo $ArrayParaSeparar[1]; ?>" id="ValorMesDoResultado" />
                          <input type="hidden" value="<?php echo $ArrayParaSeparar[2]; ?>" id="ValorAnoDoResultado" />
                          <select name="DiaDoResultado" id="DiaDoResultado" class="TextFields" onchange="document.getElementById('ValorDiaDoResultado').value = this.options[this.selectedIndex].value">
                            ...
                            <?php if($ArrayParaSeparar[0] != '00'){?>
                            <option value="<?php echo $ArrayParaSeparar[0]; ?>" selected="selected"><?php echo $ArrayParaSeparar[0]; ?></option>
                            <?php } ?>
                          ...
                          <select name="MesDoResultado" id="MesDoResultado" class="TextFields" onchange="document.getElementById('ValorMesDoResultado').value = this.options[this.selectedIndex].value">
                            ...
                            <?php if($ArrayParaSeparar[0] != '00'){?>
                            <option value="<?php echo $ArrayParaSeparar[0]; ?>" selected="selected"><?php echo $ArrayParaSeparar[1]; ?></option>
                            <?php } ?>
                            <option value="01">01</option>
                            <option value="02">02</option>
                            <option value="03">03</option>
                          ...
                          <select name="AnoDoResultado" id="AnoDoResultado" class="TextFields" onchange="document.getElementById('ValorAnoDoResultado').value = this.options[this.selectedIndex].value">
                            ...
                            <?php if($ArrayParaSeparar[0] != '0000'){?>
                            <option value="<?php echo $ArrayParaSeparar[0]; ?>" selected="selected"><?php echo $ArrayParaSeparar[2]; ?></option>
                            <?php } ?>
                      ...
                      <tr>
                        <td height="20"><!--/INICIO NOMES DAS GALERIAS/-->
                          ...
                          <select name="GaleriaDeFotosDoResultado" id="GaleriaDeFotosDoResultado" class="TextFields" onchange="document.getElementById('ValorGaleriaDeFotosDoResultado').value = this.options[this.selectedIndex].value">
                            ...
                            <?php if($BuscaMostrarResultados[1]["IDGaleriaDeFotos"] != '0'){ ?>
                            <option value="<?php echo $BuscaMostrarResultados[1]["IDGaleriaDeFotos"]; ?>" selected="selected"><?php echo utf8_encode($BuscaMostrarNomesGalerias[1]["TituloDaGaleria"]); ?></option>
                            <?php } ?>
                ...
                <tr>
                  <td height="10" align="left"><table border="0" cellspacing="3" cellpadding="0">
                      ...
                      <tr>
                        <td height="20"><input name="TituloDoResultado" type="text" class="TextFields" value="<?php echo utf8_encode($BuscaMostrarResultados[1]["TituloDoResultado"]); ?>" id="TituloDoResultado" maxlength="88" style="width:660px;" /></td>
                      </tr>
                    </table></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td height="10" align="left"><table border="0" cellspacing="3" cellpadding="0">
                      ...
                      <tr>
                        <td height="20"><input name="LocalDoResultado" type="text" class="TextFields" id="LocalDoResultado" value="<?php echo utf8_encode($BuscaMostrarResultados[1]["LocalDoResultado"]); ?>" maxlength="150" style="width:660px;" /></td>
                      </tr>
                    </table></td>
                </tr>
                <tr>
                            ...
                            <tr>
                              <td><div id="DivConteudoBBCode" style="display:block">
                                  <table border="0" cellspacing="0" cellpadding="0" class="BordaBBCode">
                                    ...
                                    <tr>
                                      <td height="20"><textarea name="TextAreaBBCode" rows="20" cols="91" class="TextAreaBBCode" id="TextAreaBBCode"><?php echo utf8_encode($BuscaMostrarResultados[1]["TextoConteudoDoResultado"]); ?></textarea></td>
                                    </tr>
                                  </table>
                                </div></td>
                            </tr>
                      ...
                      <tr>
                        <td><input type="button" class="BotaoPadrao" 
                                    onClick="
                                     ...
                                     EnviarFormularios('Ferramentas/Resultados/AcaoEditarResultado.php','DivResultadosInternos','CampoDiaDoResultado='+encodeURIComponent(document.getElementById('ValorDiaDoResultado').value)+'&CampoMesDoResultado='+encodeURIComponent(document.getElementById('ValorMesDoResultado').value)+'&CampoAnoDoResultado='+encodeURIComponent(document.getElementById('ValorAnoDoResultado').value)+'&CampoTituloDoResultado='+encodeURIComponent(document.getElementById('TituloDoResultado').value)+'&CampoLocalDoResultado='+encodeURIComponent(document.getElementById('LocalDoResultado').value)+'&CampoGaleriaDeFotosDoResultado='+encodeURIComponent(document.getElementById('GaleriaDeFotosDoResultado').value)+'&CampoTextAreaBBCode='+encodeURIComponent(document.getElementById('TextAreaBBCode').value)+'&CampoIdResultado='+encodeURIComponent('<?php echo $BuscaMostrarResultados[1]["ID"]; ?>'));
                                    }}}}}" value="Enviar"></td>
                        <td><input type="button" class="BotaoPadrao" 
