...
$SelectImagens = sprintf("SELECT cadastroimagenseventos.ID,
...
WHERE cadastroimagenseventos.PastaDeConteudoDoEvento = '%s'			
ORDER BY ID asc",
...
while ($row = mysql_fetch_array($ResultadoImagens)){			
	...
	$BuscaMostrarImagensThumbs[$nCount]["ImagemConteudoDoEvento" ] = trim($row[1]);
	$BuscaMostrarImagensThumbs[$nCount]["PastaDeConteudoDoEvento"] = trim($row[2]);
				...
				echo '<table border="0" cellpadding="0" cellspacing="0">';
						...
						for($i=1;$i<=count($BuscaMostrarImagensThumbs);$i++){
							...
							$atual = 0;
								...
								$CaminhoImagensThumbs = "../../../Img/ConteudoEventos/";																
								$CaminhoDaImagem = $CaminhoImagensThumbs."".$CampoPastaDeConteudo."/".$BuscaMostrarImagensThumbs[$i]["ImagemConteudoDoEvento"];
								$CaminhoDaImagemInsert = CAMINHO_IMAGENS_EVENTOS.''.$CampoPastaDeConteudo.'/'.$BuscaMostrarImagensThumbs[$i]["ImagemConteudoDoEvento"];
								$CaminhoDaImagemLink = substr(CAMINHO_SITE_GERAL, 7)."Img/ConteudoEventos/".$CampoPastaDeConteudo."/".$BuscaMostrarImagensThumbs[$i]["ImagemConteudoDoEvento"];
								?>
        <table border="0" cellspacing="0" cellpadding="0" id="<?php echo $BuscaMostrarImagensThumbs[$i]["ID"] ?>">
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="0" style="border:solid 2px; border-color:#FFFFFF" id="<?php echo $BuscaMostrarImagensThumbs[$i]["ID"]."Eventos" ?>">
                <tr>
                  <td><table  border="0" cellpadding="0" cellspacing="1" bgcolor="#000000">
                      <tr>
                        <td bgcolor="#FFFFFF"><a href="javascript:SelecionarImagensBBCode('<?php echo $CaminhoDaImagemLink; ?>','<?php echo $BuscaMostrarImagensThumbs[$i]["ID"]."Eventos" ?>');"><img src="Ferramentas/Eventos/MostrarThumbs.php?Imagem=<?php echo $CaminhoDaImagem; ?>" border="0" /></a></td>
                      </tr>
                    </table></td>
                </tr>
              </table></td>
  ...
  <tr>
    <td><table width="670" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="right"><input type="button" class="BotaoPadrao" value="Excluir imagens selecionadas" onClick="ConfirmacaoExclusaoPorCheckBox('Ferramentas/Eventos/AcaoExcluirImagensEvento.php','DivInserirImagem','IDImagens='+encodeURIComponent(document.getElementById('ValoresCheckbox').value)+'&CampoPastaDeConteudo='+encodeURIComponent('<?php echo $CampoPastaDeConteudo;  ?>'),'ValoresCheckbox');"/></td>
        </tr>
        <tr>
