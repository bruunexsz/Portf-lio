...
<strong>Você está editando uma MENSAGEM</strong> para os Filiados no Sistema. Toda alteração feita e salva aparecerá aos Filiados.
    </div>
</div>
...
<div class="col-12 mt-5 mb-5 p-5" style="background-color: white;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
    ...
    <form method="POST" action="{{ route('update-mensagem-controll', ['id' => $mensagem->ID]) }}">
        @csrf
        @method('PUT') <!-- Adicione o método PUT para enviar uma requisição PUT -->

        <div class="row mb-3">
            <div class="col-md-12 mb-5">
                <label for="TituloDaMensagem" class="form-label">Título da Mensagem</label>
                <input placeholder="Insira o Título da Mensagem" type="text" class="form-control" id="TituloDaMensagem" name="TituloDaMensagem" value="{{ $mensagem->TituloDaMensagem }}">
            </div>
            <hr>
...
<script>
    tinymce.init({
        ...
        ai_request: (request, respondWith) => respondWith.string(() => Promise.reject("See docs to implement AI Assistant")),
            ...
            <div class="col-md-12">
                <textarea name="TextoConteudoDaMensagem" class="form-control">{{ $mensagem->TextoConteudoDaMensagem }}</textarea>
            </div>
        </div>
